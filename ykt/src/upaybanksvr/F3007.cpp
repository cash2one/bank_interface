...
FUNCTION(3007)
{
    ...
    if(ret)
    {
        LOG(ERROR, "DB_t_ecloaddtl_read_lock_by_cur_and_accdate_and_refno \
            error,ret=" << ret);
        if(DB_NOTFOUND == ret)
        {
    ...
    if(ecloaddtl.writecardflag != 0)
    {
        ...
        {
            ERRTIP("write card flag was confirmed");
            DB_t_ecloaddtl_free_lock_by_cur();
            return  E_COMMON_ERR;
        }
    }
    ...
    int flag = (ecloaddtl.writecardflag == 1) ?
               ec_bank_trans::wc_success : ec_bank_trans::wc_failed;
