...
#define TRANSTYPE_POSDEPOSIT			211	//POS充值
#define TRANSTYPE_POSDEPOSITBOARDFEE	212	//POS充值搭伙费
...
#define TIPINFO 	CAccTrans::GetInst().SetTip
#define ERRTIP 		CAccTrans::GetInst().SetErrTip
#define ERRINFO 	getpos(__FILE__,__LINE__),CAccTrans::GetInst().SetErrMsg
//typedef struct {
//	int  cardflag;		//有无卡标志
//	int  cardtype;		//卡类型
//	int  feetype;		//收费类型
...
public:
	...
	ekingstar::REQPACK req;
    TRANS trans;
    string remark;
    char  cpackdata[2001];
    virtual ~CAccTrans(){};
	static CAccTrans* GetInstPtr();
	static CAccTrans& GetInst();
	int Reset();
//    int SaveOperdtl();
	...
	void SetErrTip(const char *errtip,...);	
	void SetFreeFlag();
	int getFreeFlag();
