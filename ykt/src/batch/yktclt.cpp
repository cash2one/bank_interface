...
void KS_YKT_Clt::SetDebug(int b)
{
  bool flag = false;
  if(b)
    flag = true;
...
bool KS_YKT_Clt::GetDoubleFieldByName(const char* FieldName, double* Value)
{
  if(!handler_)
    return false;
  return ::GetDoubleFieldByName(handler_, resp_row_, (char*)FieldName, Value);
}

bool KS_YKT_Clt::GetIntFieldByName(const char* FieldName, int* Value)
{
  if(!handler_)
    return false;
  return ::GetIntFieldByName(handler_, resp_row_, (char*)FieldName, Value);
}

bool KS_YKT_Clt::GetStringFieldByName(const char* FieldName, char* Value, int ValueBufferSize)
{
  if(!handler_)
    return false;
  return ::GetStringFieldByName(handler_, resp_row_, (char*)FieldName, Value, ValueBufferSize);
}

bool KS_YKT_Clt::SetDoubleFieldByName(const char* FieldName, double Value)
{
  if(!handler_)
    return false;
  return ::SetDoubleFieldByName(handler_, req_row_, (char*)FieldName, Value);
}

bool KS_YKT_Clt::SetIntFieldByName(const char* FieldName, int Value)
{
  if(!handler_)
    return false;
  return ::SetIntFieldByName(handler_, req_row_, (char*)FieldName, Value);
}

bool KS_YKT_Clt::SetStringFieldByName(const char* FieldName, const char* Value)
{
  if(!handler_)
    return false;
  return ::SetStringFieldByName(handler_, req_row_, (char*)FieldName, (char*)Value);
}
...
char* KS_YKT_Clt::GetReturnMsg(char* msg) const
{
  if(msg != NULL)
    strcpy(msg, err_msg_);
  return (char*)err_msg_;
}
...
bool KS_YKT_Clt::SendMsgPack(int request, const char* req, char* resp, int timeout /*= 1000*/)
{
  ::SetRawRecord(handler_, 0, (char*)req, strlen(req));
  ::SetRequestType(handler_,request);
  time_out_ = timeout;
  bool r = ::CallRequest(handler_, KS_YKT_Clt::drtp_no_,
                         0, KS_YKT_Clt::main_func_, time_out_, &err_code_, err_msg_);
  if(!r)
  {
    return false;
  }

  int rl = ::GetRawRecord(handler_,0,resp,8196);
  if( rl > 0)
  {
    resp[rl] = 0;
  }
