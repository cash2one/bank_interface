...
public:
  ...
  static void set_mainfunc(int mainfunc)
  {
    main_func_ = mainfunc;
  }
	KS_YKT_Clt();
	...
	void ResetHandler();
	bool SetIntFieldByName(const char * FieldName,int Value);
	bool SetDoubleFieldByName(const char * FieldName,double Value);
	bool SetStringFieldByName(const char * FieldName,const char * Value);
	
	bool GetIntFieldByName(const char * FieldName,int * Value);
 	bool GetDoubleFieldByName(const char * FieldName,double * Value);
  	bool GetStringFieldByName(const char * FieldName,char * Value,int ValueBufferSize);

	bool SendRequest(int request,int timeout=1000);
  bool SendMsgPack(int request,const char* req,char* resp,int timeout = 1000);
	bool HasMoreRecord();
	bool GetNextPackage();
	...
	char* GetReturnMsg(char *msg = 0) const;
